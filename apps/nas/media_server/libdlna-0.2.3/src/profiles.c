...
static char* dlna_get_mime_type(dlna_profile_t* profile)
{
	if ( profile->mime < 0 || profile->mime > DLNA_MIME_TYPE_LIST_MAX)
	{
		return dlna_mime_type_list[0];
	}
	else
	{
		return dlna_mime_type_list[profile->mime];
	}
};

static char* dlna_get_id_type(dlna_profile_t* profile)
{
	if ( profile->id < 0 || profile->id > DLNA_ID_LIST_MAX)
	{
		return dlna_id_type_list[0];
	}
	else
	{
		return dlna_id_type_list[profile->id];
	}
};
...
#ifdef USE_ORIGINAL_DLAN_PROFILE
int
stream_ctx_is_image (AVFormatContext *ctx, av_codecs_t *codecs, dlna_container_type_t st)
{
...
/* TODO. DLNA optimize. by HouXB, 20Feb12 */
char *
							...
							dlna_profile_t *p)
{
	...
	char *dlna_id = dlna_get_id_type(p);

	if (type == DLNA_PROTOCOL_INFO_TYPE_HTTP)
		sprintf (protocol, "http-get:*:");

	strcat (protocol, dlna_get_mime_type(p));
	
	if (!strcmp(dlna_id, "wav"))
	{
	...
	else
	{
		sprintf (dlna_info, ":%s=%d;%s=%d;%s=%.2x;%s=%s;%s=%.8x%.24x",
				 ...
				 "DLNA.ORG_OP", op, "DLNA.ORG_PN", dlna_id,
