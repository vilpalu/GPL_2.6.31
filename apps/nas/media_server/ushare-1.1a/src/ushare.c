...
ushare_new (void)
{
	...
	/* use the same operation to init by lyj, 10Feb12 */
	ut->model_url = strdup (DEFAULT_MODEL_URL);
	ut->manufature_url = strdup (DEFAULT_MANUFACTURE_URL);
	ut->manufature = strdup (DEFAULT_MANUFACTURE);
	ut->contentlist = NULL;
...
ushare_free (struct ushare_t *ut)
{
	...
	if (ut->interface)
		free (ut->interface);
	/* free the new param by lyj, 10Feb12 */
	if (ut->model_name)
		free (ut->model_name);
	if (ut->model_url)
		free (ut->model_url);
...
int initUshare(int argc, char **argv)
{
	...
	/* use the same operation to make free() operation ok by lyj, 10Feb12 */
	ut->name = strdup(gUshareInitInfo.serverInfo.servName);
	...
	/* use the same operation to make free() operation ok by lyj, 10Feb12 */
	ut->manufature = strdup(gUshareInitInfo.serverInfo.manufatureName);
	...
	/* use the same operation to make free() operation ok by lyj, 10Feb12 */
	ut->manufature_url = strdup(gUshareInitInfo.serverInfo.manufatureUrl);
	...
	/* use the same operation to make free() operation ok by lyj, 10Feb12 */
	ut->model_name = strdup(gUshareInitInfo.serverInfo.modelName);
	...
	/* use the same operation to make free() operation ok by lyj, 10Feb12 */
	ut->model_url = strdup(gUshareInitInfo.serverInfo.manufatureUrl);
...
#define USBDISK_PATH "/tmp/usbdisk"

/* chechk the usb device mount or not */
static int usbIsMount()
{
	DIR *dir = NULL;
	struct dirent *next = NULL;
	int num = 0;

	dir = opendir(USBDISK_PATH);
