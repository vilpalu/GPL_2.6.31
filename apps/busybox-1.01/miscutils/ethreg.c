 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/Aquila_9.2.0_U6/apps/busybox-1.01/miscutils/ethreg.c#1 $
 */
...
u_int32_t gmac_reg_write(u_int32_t reg, u_int32_t val)
{

	etd.phy_reg = reg;
	etd.cmd     = ATHR_GMAC_REG_WRITE;
	etd.val		= val;
...
u_int32_t gmac_reg_read(u_int32_t reg)
{

	etd.phy_reg = reg;
	etd.cmd     = ATHR_GMAC_REG_READ;
	if (ioctl(s,ATHR_GMAC_CTRL_IOC, &ifr) < 0)
        	err(1, etd.ad_name);
    return etd.val;
}
...
ethreg_main(int argc, char *argv[])
{
	...
	int c,portnum = 0x3f,cmd = 0,value = -1;
        ...
        int tos = -1;
	int gmacFlag = 0;
        char *opt = "xfhci:d:s:j:v:t:p:m:l:g";
	...
	for (; argc > 0; argc--, argv++) {
		u_int32_t   off;
                ...
                if (cp == NULL) {
					if (gmacFlag)
					{
						val = gmac_reg_read(off);
						printf("Read Gmac Reg: 0x%08x = 0x%08x\n", off, val);
					}
					else
					{
                	...
                	val = (u_int32_t) strtoul(cp+1, 0, 0);
                     	...
                     	else 
						{
							if (gmacFlag)
							{
								oval = gmac_reg_read(off);
								gmac_reg_write(off, val);
								printf("Write Gmac Reg: 0x%08x: Oldval = 0x%08x Newval = 0x%08x\n", off, oval, val);
								return 0;
							}
