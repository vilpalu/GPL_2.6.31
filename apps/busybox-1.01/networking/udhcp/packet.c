...
void init_header(struct dhcpMessage *packet, char type)
{
	...
	case DHCPRELEASE:
	case DHCPINFORM:
	/* fix bug 16528,when wan port gets an IP which has been obtain by
	another PC from dhcp server, our dut can't get new IP because this DECLINE
	packet has no message type, so I add it here,by chenwenhui, 2012-2-8 */
	case DHCPDECLINE:		
