...
#if WORK_KERN_VER < (KC_KERNEL_VERSION(2, 6, 27))
...
int gpl_usb_interrupt_msg(struct usb_device *usb_dev, unsigned int pipe,
						  void *data, int len, int *actual_length, int timeout)
{
  return usb_interrupt_msg(usb_dev, pipe, data, len, actual_length, timeout);
}
EXPORT_SYMBOL(gpl_usb_interrupt_msg);
