...
/* added by yangxv for syslog, 2009.12.11 */
#include <libmsglog.h>
#include <syslog.h>
		  ...
		  char const *fname)
{
	...
	/* added by yangxv for cancel *.so, 2009.12.10 */
	if (0 == strncmp(fname, "tp", sizeof("tp")))
	{
		sync_handler_init(es);
		cmd_handler_init(es);
	...
	snprintf(buf, sizeof(buf), "/tmp/%s", fname);
	buf[sizeof(buf)-1] = 0;
	handle = dlopen(buf, RTLD_NOW);
    }

	l2tp_set_errmsg("Yangxv:load handler is %s", buf);
