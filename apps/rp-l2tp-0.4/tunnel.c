...
tunnel_send_SCCRQ(l2tp_tunnel *tunnel)
{
	...
	/* BUG: 判断字符串是否有内容应该读字符指针指向的内容，这里却判断该指针是否为0，peer->hostname是静态数组，肯定不为0。
	 * 这里一般peer->hostname没有字符，所以hostname的内容为空，没有使用有内容的Hostname. 
	 * 按照RFC 2661，host name vap的value应至少有1个字节. 在某些服务器上可能因为这个而导致SCCR没有回应。
	 * tunnel->peer->hostname -> *(tunnel->peer->hostname). by lqt, 2010-3-26 */
