...
#ifdef TP_WEB_LOG
	if(memcmp(l2tp_debug_message_type_to_str(dgram->msg_type), "HELLO", 5) != 0
		&& memcmp(l2tp_debug_message_type_to_str(dgram->msg_type), "ZLB", 3) != 0)
		log_to_web("rcvd %s", l2tp_debug_message_type_to_str(dgram->msg_type));
#endif
			...
			struct sockaddr_in const *to)
{
	...
	if(memcmp(l2tp_debug_message_type_to_str(dgram->msg_type), "HELLO", 5) != 0
		&& memcmp(l2tp_debug_message_type_to_str(dgram->msg_type), "ZLB", 3) != 0)
		log_to_web("send %s", l2tp_debug_message_type_to_str(dgram->msg_type));
#endif
