...
{	
    ...
    do
	{
		...
		if (strlen(prev) >= IPT_STRING_MAX_PATTERN_SIZE)
			exit_error(PARAMETER_PROBLEM, "string match: too long '%s'\n", prev);
		
		if (count >= IPT_STRING_MAX_NUM)
			exit_error(PARAMETER_PROBLEM, "string match: too many string specified (MAX = %d)\n", IPT_STRING_MAX_NUM);
      ...
      struct ipt_entry_match **match)
{
	...
	case '4':
		...
		if (invert)
			stringinfo->invert = 1;
		#if 0
		for (j = 0; j<stringinfo->string_count; ++j)
		{
			stringinfo->patlen[j]=strlen((char *)&stringinfo->pattern[j]);
		}
		#endif
