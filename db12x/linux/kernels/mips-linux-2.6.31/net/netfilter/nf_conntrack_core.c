...
#ifdef CONFIG_ATHRS_HW_NAT
#define CONFIG_ATHRS_HW_NAT_DEBUG 0   /* To enable/disable debug prints */
#define NAT_ADD_ENTRY   0x1
#define NAT_DEL_ENTRY   0x2
#define NAT_SET_SKB_ENTRY 0x3
#define NAT_LKP_ENTRY 0x4
#endif
...
#ifdef CONFIG_ATHRS_HW_NAT
/* prototypes */
void athr_mangle_conntrack_dnat(struct nf_conn *ct, int op,int protocol,int dir);
void athr_mangle_conntrack_snat(struct nf_conn *ct, int op,int protocol,int dir);
void athr_modfiy_tb(struct nf_conn *ct, int op,int protocol,int dir, 
 				int newport,int newip, int oldport,int oldip);
struct nf_conntrack_tuple_hash *
__nf_conntrack_find_hwnat(struct net *, const struct nf_conntrack_tuple *);

/* Global defines */

typeof(athrs_hw_nat_del_entry_hook) athr_hw_nat_del_entry;
typeof(athrs_hw_nat_add_entry_hook) athr_hw_nat_add_entry;
typeof(athrs_hw_nat_get_wan_ip_addr_hook) athr_hw_nat_get_wan_ip_addr;
typeof(athrs_hw_nat_lkup_entry_hook) athr_hw_nat_lkup_entry;


#if CONFIG_ATHRS_HW_NAT_DEBUG
...
#ifdef CONFIG_ATHRS_HW_NAT
void athr_nat_add_or_del_tuple(struct nf_conn *ct, int op)
{
	uint8_t protocol;
	int maniptype = -1;
	...
	algstat = test_bit(IPS_NAT_ALG_PKT_BIT, &ct->status);
	...
	if (stat1 && stat2 && !algstat &&
		...
		if (!athr_hw_nat_add_entry ||
		    !athr_hw_nat_del_entry ||
                    !athr_hw_nat_lkup_entry ||
                    !athr_hw_nat_get_wan_ip_addr) {

                        athr_hw_nat_add_entry =
                            rcu_dereference(athrs_hw_nat_add_entry_hook);

                        athr_hw_nat_del_entry =
                            rcu_dereference(athrs_hw_nat_del_entry_hook);

                        athr_hw_nat_lkup_entry =
                            rcu_dereference(athrs_hw_nat_lkup_entry_hook);

                        athr_hw_nat_get_wan_ip_addr =
                            rcu_dereference(athrs_hw_nat_get_wan_ip_addr_hook);
...
void corrupt_tuple(struct net *net, struct nf_conn *ct,
...
//	printk("%s:%d %x\n", __func__, __LINE__, ct->tuplehash[IP_CT_DIR_HWNAT].tuple.dst.u3.ip);


	hlist_nulls_del_rcu(&ct->tuplehash[dir].hnnode);

//	if (ct->tuplehash[IP_CT_DIR_HWNAT].tuple.dst.u3.ip == 0) {
		ct->tuplehash[IP_CT_DIR_HWNAT] = ct->tuplehash[dir];
		ct->tuplehash[IP_CT_DIR_HWNAT].tuple.src.u.all = 0x0;
		ct->tuplehash[IP_CT_DIR_HWNAT].tuple.src.u3.ip = 0x0;
		repl_hash = hash_conntrack(&ct->tuplehash[IP_CT_DIR_HWNAT].tuple);
		hlist_nulls_add_head_rcu(&ct->tuplehash[IP_CT_DIR_HWNAT].hnnode,
		                         &net->ct.hwnat_mod_hash[repl_hash]);
//	}
...
void revert_tuple(struct net *net, struct nf_conn *ct,
                  enum ip_conntrack_dir dir,
                  uint32_t ip, uint16_t port)
{
	...
	h = __nf_conntrack_find_hwnat(net,
                                      &ct->tuplehash[IP_CT_DIR_HWNAT].tuple);

	if (h) {
	    hlist_nulls_del_rcu(&ct->tuplehash[IP_CT_DIR_HWNAT].hnnode);
	}
...
void athr_mangle_conntrack_snat(struct nf_conn *ct, int op,
				int protocol,int dir)
{
	...
	athr_modfiy_tb(ct,op,protocol,dir,newport,newip,oldport,oldip);
}
...
void athr_mangle_conntrack_dnat(struct nf_conn *ct, int op,int protocol,int dir) 
{
	...
	uint16_t port;
		...
		if (op == NAT_ADD_ENTRY) {
                        ret = athr_hw_nat_lkup_entry(newip, newport,
                                                      NAT_INGRESS,
                                                      protocol,
                                                      &ipaddr, &port);
                        if ((ret != 0) ||
                    ...
                    int newport,int newip, int oldport,int oldip)
{
	...
	if (op == NAT_ADD_ENTRY) {
		...
		if ((dir == NAT_EGRESS) ||
			...
			ct->tuple[0].src.u3.ip = oldip;
                        ct->tuple[0].dst.u3.ip = newip;
                        ct->tuple[0].src.u.all = oldport;
                        ct->tuple[0].dst.u.all = newport;
                        ct->tuple[0].dst.protonum = protocol;
                        ct->tuple[0].dst.dir = NAT_EGRESS;

			ret = athr_hw_nat_lkup_entry(oldip, oldport,
                                                      NAT_EGRESS, protocol,
                                                      &ipaddr, &port);
			if (ret) {
				ret = 0;
                                ret |= athr_hw_nat_add_entry(oldip, oldport, newip,
		...
		if (((dir == NAT_INGRESS) ||
			...
			ct->tuple[1].src.u3.ip = newip;
                        ct->tuple[1].dst.u3.ip = oldip;
                        ct->tuple[1].src.u.all = newport;
                        ct->tuple[1].dst.u.all = oldport;
                        ct->tuple[1].dst.protonum = protocol;
                        ct->tuple[1].dst.dir = NAT_INGRESS;
			...
			if (ret) {
				ret = 0;
				ret |= athr_hw_nat_add_entry(newip, newport, oldip,
		...
		revert_tuple(net, ct, c_dir, ct->newip, ct->newport);
		...
		if (!h) {
			if ((dir == NAT_EGRESS) ||
        			...
        			athr_hw_nat_del_entry(oldip, oldport, protocol,
                                                      NAT_EGRESS);
			}
			...
			if ((dir == NAT_INGRESS) ||
				...
				if(test_bit(IPS_ATHR_HW_NAT_IS_UDP_BIT, &ct->status))
        			...
        			athr_hw_nat_del_entry(newip, newport, protocol,
                                                      NAT_INGRESS);
			}
		}
...
__nf_conntrack_find_hwnat(struct net *net, const struct nf_conntrack_tuple *tuple)
{
	...
	local_bh_disable();
begin:
	hlist_nulls_for_each_entry_rcu(h, n, &net->ct.hwnat_mod_hash[hash], hnnode) {
...
EXPORT_SYMBOL_GPL(__nf_conntrack_find_hwnat);
#endif
				   ...
				   gfp_t gfp)
{
	...
	memset(&ct->tuplehash[IP_CT_DIR_HWNAT], 0,
	       sizeof(*ct) - offsetof(struct nf_conn, tuplehash[IP_CT_DIR_HWNAT]));
	       ...
	       )
{
	...
	{
		tuple->dst.u3.ip = athr_hw_nat_get_wan_ip_addr();
        }
#endif
		  ...
		  enum ip_conntrack_info *ctinfo)
{
	...
	if (!h) {
#ifdef CONFIG_ATHRS_HW_NAT
		h = init_conntrack(net, &tuple, l3proto, l4proto, skb, dataoff, protonum);
#else
...
#ifdef CONFIG_ATHRS_HW_NAT
	uint8_t protocol;
//    struct iphdr *ip_pkt =  (struct iphdr *)((u8 *)skb->data+14);
#endif
...
#ifdef CONFIG_ATHRS_HW_NAT
	nf_ct_free_hashtable(net->ct.hwnat_mod_hash, net->ct.hwnat_mod_hash_vmalloc,
			     nf_conntrack_htable_size);
#endif
...
#ifdef CONFIG_ATHRS_HW_NAT
	net->ct.hwnat_mod_hash = nf_ct_alloc_hashtable(&nf_conntrack_htable_size,
					     &net->ct.hwnat_mod_hash_vmalloc, 1);
	if (!net->ct.hwnat_mod_hash) {
		ret = -ENOMEM;
		printk(KERN_ERR "Unable to create nf_conntrack_hash\n");
		goto err_hw_hash;
	}
