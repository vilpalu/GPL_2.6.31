...
#define CONFIG_S17_SPI
#else
#undef CONFIG_S17_SPI
#endif

#ifdef CONFIG_S17_SPI
/* jhung, SPI */
int sysctl_spi_enable = 0; 
extern bool athrs17_tcp_in_window(const struct nf_conn *ct,
	struct ip_ct_tcp *state, enum ip_conntrack_dir dir,
			  ...
			  u_int8_t pf)
{
	...
	bool res;
#ifdef CONFIG_S17_SPI /* jhung, SPI */
	struct iphdr *iph;
...
#ifdef CONFIG_S17_SPI
		if( dev && (dev->name) && (strcmp(dev->name, "br0") != 0) ){
#if 0
			if (!before(sack, receiver->td_end + 1) && \
				((tcph->fin) || (tcph->ack))) 
		 	{ /* Chariot start with HNAT, or normal handshake packets */
		 		res = true;
			}else if (!before(seq, sender->td_maxend + 1) && \
				!before(sack, receiver->td_end + 1) && \
				(tcph->fin) && (tcph->rst)) 
			{ /* Chariot end with HNAT */
				res = true;
			}else 
#endif			
			if( ! before(sack, receiver->td_end + 1) || \
		 	...
		 	{
		 		if ( net_ratelimit() )
		 			printk(KERN_NOTICE "Blocked incoming TCP Ack packet from %u.%u.%u.%u:%hu to %u.%u.%u.%u:%hu with unexpected sequence number\n" \
		 	                	,NIPQUAD(iph->saddr), ntohs(tcph->source), NIPQUAD(iph->daddr), ntohs(tcph->dest));
				printk("unknown TCP control packets... ");
				printk("tcp_in_window: I=%i II=%i III=%i IV=%i\n",
					before(seq, sender->td_maxend + 1),
					after(end, sender->td_end - receiver->td_maxwin - 1),
					before(sack, receiver->td_end + 1),
					after(sack, receiver->td_end - MAXACKWINDOW(sender) - 1));

				printk("syn: %i, fin: %i, ack: %i, rst: %i\n", \
		 			(tcph->syn ? 1:0), (tcph->fin ? 1:0), (tcph->ack ? 1:0), \
		 			(tcph->rst ? 1:0) ); 
			}
		}
#if 0
		printk("other packets... ");
		printk("tcp_in_window: I=%i II=%i III=%i IV=%i\n",
			before(seq, sender->td_maxend + 1),
			after(end, sender->td_end - receiver->td_maxwin - 1),
			before(sack, receiver->td_end + 1),
		...
		if(sysctl_spi_enable){
			if( (index == TCP_SYNACK_SET) && \
				(ct->proto.tcp.last_index != TCP_SYN_SET) && \
		 	        ...
		 	        (dev->name) && \
		 	        (strcmp(dev->name, "br0") != 0) )
			{
		 		if ( net_ratelimit() )
		 			printk(KERN_NOTICE "Blocked incoming TCP Syn+Ack packet from %u.%u.%u.%u:%hu to %u.%u.%u.%u:%hu with unexpected sequence\n" \
